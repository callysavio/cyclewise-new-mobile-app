  ...
  return (
    ...
    >
      <View style={modalStyles.centeredView}>
        <View
          style={[modalStyles.modalView, { backgroundColor: theme.background }]}
        >
          <Text style={[modalStyles.modalTitle, { color: theme.text }]}>
            {title}
          </Text>
          <Text style={[modalStyles.modalText, { color: theme.icon }]}>
            {message}
          </Text>
          <TouchableOpacity
...
const CustomButton: React.FC<ButtonProps> = ({
  ...
  return (
    <TouchableOpacity
      ...
      style={[styles.button, { backgroundColor: theme.tint, opacity }]}
    >
      {loading ? (
    ...
    <View style={styles.inputContainer}>
      ...
      <View style={pickerStyles.pickerInputWrapper}>
        <TextInput
          ...
          textAlign="center"
        />
        <Text style={[pickerStyles.unitText, { color: theme.icon }]}>
          {unit}
        </Text>
      </View>
      <Text
        style={[
          styles.infoText,
          { color: theme.icon, marginTop: 5, fontSize: 12 },
        ]}
      >
        (Range: {min}-{max} {unit})
      </Text>
...
const CycleStartScreen: React.FC<SetupScreenProps> = ({
  state,
  setState,
  setCurrentScreen,
  showMessage,
  ...
  const handleDateSelect = (day: number) => {
    ...
    if (selectedDateObj.getTime() > todayMidnight.getTime()) {
      showMessage(
        "Future Date",
        "You can't select a start date in the future.",
      );
      return;
    }
  ...
  const isSelected = (day: number) => {
    ...
    const [selectedYear, selectedMonth, selectedDay] = state.cycleStart
      .split("-")
      .map(Number);
    ...
    return (
      ...
      >
        ...
        <ScrollView
          ...
          style={calendarStyles.yearScrollViewContent}
        >
          {years.map((year) => (
            <TouchableOpacity
              key={year}
    ...
    <View style={styles.screenContent}>
      ...
      <View style={calendarStyles.calendarContainer}>
        ...
        <View style={calendarStyles.monthHeaderContainer}>
          ...
          >
            <Text
              style={[calendarStyles.navText, { color: theme.headerControl }]}
            >
              &lt;
            </Text>
          </TouchableOpacity>
              ...
              <Text
                style={[
                  calendarStyles.monthHeader,
                  {
                    ...
                    textDecorationLine: isMonthPickerVisible
                      ? "none"
                      : isYearPickerVisible
                        ? "none"
                        : "underline",
              ...
              <Text
                style={[
                  calendarStyles.monthHeader,
                  {
                    color: theme.text,
                    // Underline only the active title or when no picker is open
                    textDecorationLine: isYearPickerVisible
                      ? "none"
          ...
          <TouchableOpacity
            ...
            style={[
              ...
              ((currentMonth === today.getMonth() &&
                currentYear === today.getFullYear()) ||
                isMonthPickerVisible ||
                isYearPickerVisible) && { opacity: 0.4 },
            ]}
          >
            <Text
              style={[calendarStyles.navText, { color: theme.headerControl }]}
          ...
          <>
            <View style={calendarStyles.dayNamesContainer}>
              {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => (
                <Text
                  key={day}
                  style={[calendarStyles.dayName, { color: theme.icon }]}
                >
                  {day}
                </Text>
              ))}
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      style={[
                        ...
                        selected && {
                          backgroundColor: theme.dateSelectedBg,
                          borderRadius: 9999,
                        },
                    ...
                    >
                      <Text
                        style={[
                          ...
                          selected
                            ? { color: theme.dateSelectedText }
                            : { color: theme.dateUnselectedText },
...
const CycleLengthScreen: React.FC<SetupScreenProps> = ({
  state,
  setState,
  setCurrentScreen,
}) => {
  ...
  return (
    <View style={styles.screenContent}>
      ...
      <Text style={[styles.subheader, { color: theme.text, marginTop: 40 }]}>
        Cycle Setup
      </Text>
      <Text style={[styles.infoText, { color: theme.icon, marginBottom: 40 }]}>
        Let&apos;s understand your cycle - tell us a bit about your period to
      ...
      <View style={{ alignItems: "center", marginVertical: 40 }}>
        <NumericInput
          ...
          onChange={(num) =>
            setState((prev) => ({ ...prev, cycleLength: num }))
          }
          placeholder="28"
          min={21}
          max={35}
      ...
      <CustomButton
        ...
        onPress={handleNext}
        disabled={
          !state.cycleLength || state.cycleLength < 21 || state.cycleLength > 35
        }
      />
    </View>
  );
...
const PeriodDurationScreen: React.FC<SetupScreenProps> = ({
  state,
  setState,
  setCurrentScreen,
}) => {
    ...
    <View style={styles.screenContent}>
      ...
      <View style={{ alignItems: "center", marginVertical: 40 }}>
        <NumericInput
          ...
          onChange={(num) =>
            setState((prev) => ({ ...prev, periodDuration: num }))
          }
          placeholder="5"
          min={3}
          max={7}
      ...
      <CustomButton
        ...
        disabled={
          ...
          state.periodDuration < 3 ||
          state.periodDuration > 7
        }
      />
    </View>
  );
...
const SetupCompleteScreen: React.FC<SetupScreenProps> = ({
  state,
  setCurrentScreen,
}) => {
  const theme = useTheme();
  ...
  return (
    ...
    >
      <Text
        style={[
          styles.subheader,
          { color: theme.text, fontSize: 32, textAlign: "center" },
        ]}
      >
        ✅ Setup Complete!
      </Text>
      <Text
        style={[
          styles.infoText,
          { color: theme.icon, marginVertical: 20, textAlign: "center" },
        ]}
      >
        Thank you for setting up your cycle details. We&apos;re ready to
        personalize your experience!
      </Text>
      <View style={{ padding: 10 }}>
        ...
        <Text style={[styles.infoText, { color: theme.icon }]}>
          Last Period Start: {state.cycleStart || "N/A"}
        </Text>
        <Text style={[styles.infoText, { color: theme.icon }]}>
          Cycle Length: {state.cycleLength || "N/A"} days
        </Text>
        <Text
          style={[styles.infoText, { color: theme.icon, marginBottom: 20 }]}
        >
          Period Duration: {state.periodDuration || "N/A"} days
        </Text>
      </View>
      <TouchableOpacity
        onPress={() => setCurrentScreen("CycleStart")}
        style={{ padding: 10 }}
      >
        <Text
          style={[
            styles.linkText,
            { color: theme.tint, fontWeight: "bold", fontSize: 16 },
          ]}
        >
          Restart Setup
        </Text>
  ...
  return (
    ...
    >
      ...
      <KeyboardAvoidingView
        ...
        behavior={
          Platform.OS === "ios" || Platform.OS === "web" ? "padding" : "height"
        }
      >
        <View style={styles.innerContainer}>{renderScreen}</View>
      </KeyboardAvoidingView>
    </SafeAreaProvider>
  );
};
